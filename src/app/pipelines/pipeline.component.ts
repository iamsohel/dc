  ...
  template: `
    ...
    <ng-container *ngIf="_loader.loaded">
      ...
      <div class="row">
        ...
        <div class="col-md-6">
          <div class="form-group p0 pull-right">
...
export class PipelineComponent implements OnInit, OnDestroy, HasUnsavedData {
  ...
  constructor(
    ...
    this._loader = new ReactiveLoader((id: TObjectId | null) => {
      return forkJoin(
        id ? this._pipelineService.get(id) : of(null),
        this._pipelineService.listAllOperators(),
      );
    });
  ...
  ngOnInit(): void {
    const paramsSubscription = this._activatedRoute.params.subscribe((params: Params) => {
      this._loader.load('pipelineId' in params ? params['pipelineId'] : null);
    });
  ...
  _onPipelineLoaded([pipeline, operators]: [Pipeline | null, PipelineOperator[]]): void {
    if (pipeline) {
