  ...
  ngOnInit() {
    this.routeSubscription = this.route.params.subscribe(params => {
      ...
      if (!typeFilter || !scopeOrItemId) {
        const fallbackAssets = this.assets.filter(
          asset => this.featureService.areFeaturesEnabled(this.sections[asset].features),
        );
        const fallbackAsset = fallbackAssets.indexOf(IAsset.Type.TABLE) !== -1 ? IAsset.Type.TABLE : fallbackAssets[0];
        ...
        this.router.navigate(['/desk', 'library', typeFilter, 'personal', scopeOrItemId], {replaceUrl: true});
      } else {
