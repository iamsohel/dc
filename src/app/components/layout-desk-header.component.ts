...
-->
        ...
        <li [class.active]="pmeSection.isActive"
        [routerLinkActive]="['active']"
        *ngIf="user.getUser() | apply: _userHasPmeAccess">
          <a [routerLink]="['/desk', 'pme']">PME</a>
        </li>
      </ul>
      <ul class="nav navbar-nav navbar-right">
        <li class="nav-item" [ngClass]="{disabled: false}">
          <a class="nav-link" href="https://deepcortex.ai/pme-user-manual/" target="_blank" title="User Manual">
            <i class="iconapp iconapp-rotate"></i>
          </a>
        </li>
  ...
  <nav class="navbar navbar-default nav-header brand-header navbar-conditional" role="navigation" [routerLinkActive]="['active']">
    <div class="navbar-collapse navbar-ex1-collapse">
      ...
      <ul class="nav navbar-nav navbar-conditional navbar-left" [routerLinkActive]="['active']" #pmeSection="routerLinkActive">
        <!-- <li [routerLinkActive]="['active']">
          <a [routerLink]="['/desk', 'pme']">Price Management</a>
        </li> -->
      </ul>
    </div>
  </nav>
...
export class LayoutDeskHeaderComponent {
  ...
  _userHasPmeAccess(user: IUser) {
    return (user.role === UserRole.ADMIN || user.role === UserRole.SALES_REP || user.role === UserRole.SALES_MANAGER || user.role === UserRole.GMM || user.role === UserRole.CD || user.role === UserRole.GPO_ANALYST);
  }
}
