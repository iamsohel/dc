...
@Component({
  ...
  template: `
    ...
    <ng-container *ngIf="!(isLoading$ | async)">
      ...
      <publish-dc-project-modal #publishModal></publish-dc-project-modal>
    </ng-container>
  `,
})
export class PackagesListComponent implements OnChanges, OnDestroy {
  ...
  @ViewChild('publishModal') publishModal: PublishDcProjectModalComponent;
  ...
  constructor(
    ...
    this._subscription.add(this.eventService.subscribe((event) => {
      ...
      if (
        ...
        && (!this.searchParams.dcProjectId || this.searchParams.dcProjectId === event.data.targetId)
      ) {
        this._loadPackages();
      }
    }));
