  ...
  template: `
    ...
    >
      <ng-container *ngIf="selectedItems.length === 1">
        <p>Are you sure you want to delete DC Project Package: <strong>{{(selectedItems[0].name)}} {{selectedItems[0].version ? '(' + selectedItems[0].version + ')' : ''}}</strong></p>
      </ng-container>

      <ng-container *ngIf="selectedItems.length > 1">
        <p>Are you sure you want to delete these DC Project Packages:</p>
        <ul>
