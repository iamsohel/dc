            ...
            <dl>
              ...
              <ng-container *ngIf="model.modelType | apply: asCustomModelType as modelTypeCustom">
                ...
                <dd *ngVar="modelTypeCustom.classReference as classReference">
                  ...
                  {{classReference.moduleName}}/{{classReference.className}}<span *ngIf="customModelPackage"> ({{customModelPackage.name}})</span>
                </dd>
              </ng-container>
            </dl>
          </div>
...
export class CVModelViewEmbedComponent implements OnInit, OnChanges {
  ...
  readonly _customModelPackageLoader: ReactiveLoader<IPackage, TObjectId>;

  architectures: ICVArchitecture[];
  classifiers: ICVClassifier[];
  detectors: ICVDetector[];
  customModelPackage: IPackage = null;
  ...
  constructor(
    ...
    this._customModelPackageLoader = new ReactiveLoader(packageId => packagesService.get(packageId));
    this._customModelPackageLoader.subscribe(p => this.customModelPackage = p);
  }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if ('model' in changes) {
      const customType = this.asCustomModelType(this.model.modelType);
      this.customModelPackage = null;
      if (customType && customType.classReference && customType.classReference.packageId) {
        this._customModelPackageLoader.load(customType.classReference.packageId);
      }
    }
  }
