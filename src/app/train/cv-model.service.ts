...
import { ICVModel, ICVModelClone, ICVModelImport, ICVModelUpdate } from './cv-model.interface';

@Injectable()
export class CVModelService
  implements IAssetService<ICVModel, null>,
    IAssetService.Importable<ICVModel, ICVModelImport>,
    IAssetService.Cloneable<ICVModel, ICVModelClone> {
  ...
  clone(id: TObjectId, clone: ICVModelClone) {
    if (clone.name === '') {
      delete clone.name;
    }
    const observable = this.sharedService.withSharedAccess(
      IAsset.Type.CV_MODEL,
      id,
    ).post('cv-models/' + id + '/copy', clone);

    return AppHttp.execute(observable,
      (model: ICVModel) => {
        this.events.emit(IEvent.Type.CREATE_CV_MODEL, model);
        this.events.emit(IEvent.Type.UPDATE_CV_MODEL_LIST);
...
declare module '../core/services/event.service' {
  export namespace IEvent {
    export const enum Type {
      CREATE_CV_MODEL = 'CREATE_CV_MODEL',
