      ...
      <div class="panel">
        <div class="panel-body" [ngSwitch]="item.target.type">
          <dl class="dl-horizontal" *ngSwitchCase="'${IAsset.Type.PIPELINE}'">
            <dt>API URL</dt>
            <dd>https://dev.deepcortex.ai/api-call/{{item.id}}/run</dd>
            <dt>Sample call</dt>
            <dd>
              <div class="row">
                <div class="col-sm-6">
          ...
          <dl class="dl-horizontal" *ngSwitchCase="'${IAsset.Type.MODEL}'">
            <dt>API URL</dt>
            <dd>https://dev.deepcortex.ai/api-call/{{item.id}}/predict</dd>
            <dt>Sample call</dt>
            <dd>
...
export class OnlineAPIViewComponent implements OnInit, OnDestroy {
  ...
  readonly _modelLoader: ReactiveLoader<IModel, TObjectId>;
  ...
  constructor(
    ...
    this._modelLoader.subscribe((model: IModel) => {
      this.model = model;
    });
  }
  ...
  protected _prepareSamplePredictRequest = (model: IModel): any => {
    const dataSample = {'name': model.name};
    return {
      data: [dataSample],
    };
  };

  protected _prepareSamplePredictResponse = (model: IModel): any => {
    const dataSample = {'name': model.name};
    return {
