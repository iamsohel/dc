...
import { IModelHelpers } from '../train/model.interface';
              ...
              <tr>
                <th class="text-center"
                  ...
                  *ngFor="let item of table.columns"
                  ...
                  data-container="body"
                  [tooltipTitle]="tableStatisticTooltips[item.name] || ''"
          ...
          <table *ngSwitchCase="true" class="table table-dataset">
            ...
            <tr *ngFor="let item of table.columns; let i = index" [ngClass]="{
              ...
              >
                ...
                <ng-template ngSwitchDefault>{{row[i].toString() | truncate: 50}}</ng-template>
              </td>
            </tr>
          </table>
        </div>
...
export class TableViewEmbeddableComponent extends TableViewCommon implements OnChanges, OnInit, OnDestroy {
  @Input() id: TObjectId;
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    this._getType = (item: ITableColumn, key: string) => { // or dataType
      ...
      if (this.modelHelpers) {
        let index = _.findIndex(this.modelHelpers.predictorColumns, _ => _.name === item.name);
        if (index > -1) {
