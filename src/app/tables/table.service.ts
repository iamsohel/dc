...
import { IS3BucketReference } from '../core/core.interface';
...
export interface ITableImportFromS3 extends ITableImport, IS3BucketReference {
  filePath: string;
}
...
export class TableService extends DataService
  ...
  versions(parentId: TObjectId, params?: IAssetListRequest): Observable<IBackendList<ITable>> {
    const observable = this.http.get(`tables/${parentId}/versions`, params, {
      deserialize: _ => this._patchTables(_),
    });
  ...
  saveVersion(parentId: TObjectId, file: File, params: ITableImport): Observable<ITable> {
    const observable = this.http.monitoredUpload(`tables/${parentId}/save-version/csv`, file, {
      parentId: parentId,
  ...
  'delete'(item: ITable): Observable<ITableId> {
    ...
    let notificationText = !!item.name
    ...
    return AppHttp.execute(observable,
      (data: ITableId) => {
        ...
        this.notifications.create('Table deleted: ' + notificationText);
      },
    );
  }
  ...
  importFromS3(params?: ITableImportFromS3): Observable<ITable> {
    const format = params.format;
    const body = {
      filePath: params.filePath,
      name: params.name,
      description: params.description,
      AWSS3BucketId: params.AWSS3BucketId,
      AWSRegion: params.AWSRegion,
      AWSS3BucketName: params.AWSS3BucketName,
      AWSAccessKey: params.AWSAccessKey,
      AWSSecretKey: params.AWSSecretKey,
      AWSSessionToken: params.AWSSessionToken,
    };
    if (format === 'csv') {
      body['delimiter'] = params.delimiter;
      if (params.nullValue) {
        body['nullValue'] = params.nullValue;
      }
    }
    const observable = this.http.post(`tables/importFromS3/${format}`, body)
      .flatMap((table: ITable) => this.get(table.id));
    ...
    return AppHttp.execute(observable,
      table => {
        this.events.emit(IEvent.Type.CREATE_TABLE, table);
        this.events.emit(IEvent.Type.UPDATE_TABLE_LIST);
        this.notifications.create('Import Complete');
      },
    );
  }
  ...
  generateColumn(index: number, columnCount: number): object {
    return {
      headerName: this.generateColumnLetter(index + 1),
      field: this.generateColumnField(columnCount),
    };
  }

  generateColumnField(columnCount: number): string {
    return `column${columnCount}`;
  }

  generateColumnLetter(num: number): string {
    let mod = num % 26;
    let pow = Math.floor(num / 26);
    let out = mod ? String.fromCharCode(64 + mod) : 'Z';
    if (!mod) {
      pow--;
    }
    return pow ? this.generateColumnLetter(pow) + out : out;
  }
