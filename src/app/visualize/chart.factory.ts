...
export namespace ChartFactory {
  ...
  export function createOptionsComponentInstance(
    viewContainer: ViewContainerRef,
    componentFactoryResolver: ComponentFactoryResolver,
    charType: IDashboard.DashboardChartType,
    options: TChartOptions,
    attributes: IColumnInfo[],
  ): ComponentRef<ChartOptionsAbstract> {
    const optionsClass = ChartFactory.getOptionsClass(charType);
    if (!optionsClass) {
      return;
    }
    ...
    compRef.instance.attributes = attributes;
    return compRef;
  }
  ...
  export function createComponentInstance(
    ...
    dataLoader: (req: TabularDataRequest) => Observable<TabularDataResponse>,
  ): ComponentRef<ChartAbstract<any>> {
    const factory = componentFactoryResolver.resolveComponentFactory(getComponentClass(config.type));
    const compRef = viewContainer.createComponent(factory);
    compRef.instance.dataLoader = dataLoader;
