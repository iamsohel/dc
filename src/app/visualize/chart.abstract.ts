...
export abstract class ChartAbstract<T extends DashboardCharts.IChartOptions> implements OnInit, OnDestroy {
  ...
  set dataLoader(loader: (request: TabularDataRequest) => Observable<TabularDataResponse>) {
    if (this._dataLoader) {
      throw new Error('Data loader cannot be set twice');
    }
    this._dataLoader = new ReactiveLoader(loader);
  ...
  getData() {
    if (!this._config.metrics /*|| !this._config.attributes*/ || !this._config.metrics.length /*|| !this._config.attributes.length*/) {
  ...
  download() {
    ...
    MiscUtils.downloadUrl(url, `${this.table ? this.table.name : 'table'}.csv`);
  }
