...
import { IDimensions, TTableValue } from '../tables/table.interface';
...
import { DashboardEditState, IInputInfo } from './dashboard-edit-state';
      ...
      <div class="row">
        <div class="col-xs-6">
          ...
          <div *ngIf="state.widgetForm.value.input?.type === config.asset.values.TABLE"
            ...
            <chart-edit-filters
              [dimensions]="state.widgetForm.controls.input.value | apply: _getDimensions"
              [widgetForm]="state.widgetForm"
              [crossFilters]="state.form.controls.crossFilters.value"
              (crossFiltersChange)="state.form.controls.crossFilters.setValue($event)"
            ></chart-edit-filters>
          </div>
          ...
          <div *ngIf="state.widgetForm.value.input?.type === config.asset.values.TABLE"
            ...
            <chart-edit-groups
              [widgetForm]="state.widgetForm"
              [widgetAttributeColumns]="state.getWidgetAttributeColumns()"
              [columnValuesLoader]="state | apply: _getColumnValuesLoader: _tables"
            ></chart-edit-groups>
          </div>
        </div>
        <div class="col-xs-6">
...
export class ChartEditComponent implements OnChanges, OnDestroy {
  ...
  private syncFromFormValue(widget: IDashboardWidget) {
    ...
    if (!oldConfig || !_.isEqual(oldConfig.type, newConfig.type)) {
      ...
      this.compRef = ChartFactory.createComponentInstance(
        ...
        (request) => this.dataFetch.fetchTabularData(request).debounceTime(0),
      );
      this.optsRef && this.optsRef.destroy();
      this.optsRef = ChartFactory.createOptionsComponentInstance(
        this.chartOptions,
        this.componentFactoryResolver,
        this.state.widgetForm.value.type,
        this.state.widgetForm.value.options,
        this.state.getWidgetAttributeColumns(),
      );
  ...
  private _getDimensions(input: IInputInfo): IDimensions {
  ...
  private checkHierarchy(attributes: string[]) {
    ...
    if (currentInput) {
      ...
      if (options.hierarchy !== isHierarchy && isHierarchy) {
        ...
        this.state.widgetForm.controls['options'].setValue(options);
        if (this.optsRef) {
          this.optsRef.instance.options = options;
        }
      }
    }
  }

  private _getColumnValuesLoader(
    state: DashboardEditState,
    tableService: TableService,
  ): (attribute: string) => Observable<{ columnName: string; values: TTableValue[] }> {
    const input = state.getCurrentInput();
    return (attribute: string) => {
      return tableService.values(input.table.id, {
        column_name: attribute,
        search: '',
        limit: 100,
      }).map((_): { columnName: string; values: TTableValue[] } => {
        return {
