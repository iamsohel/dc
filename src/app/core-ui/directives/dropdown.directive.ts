...
export class DropDownDirective implements OnInit, OnDestroy {
  ...
  private _onShow(eventObject: JQuery.Event) {
    ...
    const fitToContainer = container &&
      ( !this._tendsToEqual(container.scrollHeight, container.offsetHeight) ||
        !this._tendsToEqual(container.scrollWidth, container.offsetWidth) ||
  ...
  // there are some browser related issues so we are tolerating 1 pixel difference
  private _tendsToEqual(numberOne: number, numberTwo: number) {
