...
export class AppSelectComponent implements OnDestroy, AfterViewInit, OnChanges {
  ...
  private defaultOptions = {
    ...
    onDropdownShow: () => {
      ...
      if (this.optionsProvider) {
        this.optionsProviderLoader.load();
      }
    },
    onDropdownHide: () => {
  ...
  private _preparedOptions: InternalSelectOptionData[] = [];
  private optionsProviderLoader: ReactiveLoader<AppSelectOptionsType, void>;
  ...
  constructor(
    ...
    this.optionsProviderLoader = new ReactiveLoader<AppSelectOptionsType, void>(
      () => this.optionsProvider ? this.optionsProvider() : empty(),
    );
    this.optionsProviderLoader.subscribe(options => {
      this.options = options;
      this.onOptionsUpdate();
    });
    this.optionsProviderLoader.active.subscribe(isActive => {
      if (isActive) {
        this.zone.runOutsideAngular(() => {
          if (this.element ) {
            this.element.multiselect('dataprovider', [
              { value: null, label: 'Loading...', selected: true, disabled: true},
            ]);
          }
        });
      }
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes.hasOwnProperty('optionsProvider')) {
      if (this.optionsProvider) {
        this.optionsProviderLoader.load();
      }
    }
  ...
  ngOnDestroy() {
    this.optionsProviderLoader.complete();
    this.destroyPlugin();
    this.controlSubscription && this.controlSubscription.unsubscribe();
  }
  ...
  private _prepareOptionsMapping() {
    type ReducerResult = [InternalSelectOptionData[], {[k: string]: AppSelectOptionData.IdType}];
    const reducer = (options: AppSelectOptionsType, prefix: string = ''): ReducerResult => {
      return options.reduce<ReducerResult>(([accOptions, accMap], option, idx) => {
        ...
        if (AppSelectOptionData.isOptionData(option)) {
          const out: InternalSelectOptionData = { value: id, label: option.text, disabled: option.disabled };
          accOptions.push(out);
          accMap[id] = option.id;
