...
export class AppValidators {
  ...
  static validDate(dateField: string, errorName: string = 'validDate'): ValidatorFn {
    return (formGroup: AbstractControl): { [key: string]: boolean } | null => {
      const isValid = AppValidators._validators.date(formGroup.get(dateField));
  ...
  static validNumber(control: FormControl, errorName: string = 'validNumber') {
    if (AppValidators._validators.float(control)) {
      return {
        errorName: {
          valid: false,
        },
