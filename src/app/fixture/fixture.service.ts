...
import { ICd, ICustomer, IGmm, IProduct, IRegion, ISalesManager, ISalesRepresentative } from '../pme/pme.interfaces';
...
export type FixtureServiceCollectionsMap = {
  ...
  tabularModels: Loki.Collection<IFixtureTabularModel>;
  models: Loki.Collection<IModel>;
  ...
  dcProjects: Loki.Collection<IDCProject>;
  packages: Loki.Collection<IPackage>;
  dcProjectSessions: Loki.Collection<IDCProjectSession>;
  ...
  pricerecords: Loki.Collection<IFixturePriceRecord>;
  customers: Loki.Collection<ICustomer>;
  products: Loki.Collection<IProduct>;
  regions: Loki.Collection<IRegion>;
  salesManagers: Loki.Collection<ISalesManager>;
  salesRepresentatives: Loki.Collection<ISalesRepresentative>;
  cds: Loki.Collection<ICd>;
  gmms: Loki.Collection<IGmm>;
};
  ...
  private initProcesses(): void {
    ...
    window.setInterval(() => {
      processes.findAndUpdate(item => item.status === IProcess.Status.RUNNING && item.progress < 1, process => {
        ...
        let progress = process.progress || 0;
        ...
        if (process.progress >= 1) {
          ...
          if (process.jobType === IProcess.JobType.PROJECT_BUILD) {
            ...
            const dcProject = dcProjects.findOne({ id: process.targetId });
            if (dcProject) {
              dcProject.status = IDCProject.Status.IDLE;
              dcProjects.update(dcProject);
            }
          }
          ...
          if (process.target === IAsset.Type.MODEL) {
            ...
            const model = models.findOne({ id: process.targetId });

            if (model) {
              model.status = IModel.Status.ACTIVE;
              models.update(model);
            }
          }
