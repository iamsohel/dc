...
export const tablesRoutes: IFixtureServiceRoute[] = [
  ...
  {
    url: 'tables/importFromS3/(csv|json)$',
    method: 'POST',
    handler: function(this, params, user) {
      ...
      const datasets = this.collections.datasets;
      if (!tables.count()) {
        throw new Error('Error while importing: Mocked tables list is empty');
      }
      const duplicateTable = tables.find({name: newName, ownerId: user.id});
      if (duplicateTable.length) {
        throw new Error(`Table with name "${newName}" already exists`);
      }
      const sourceTable = tables.chain().limit(1).data()[0];
      return getDataset(this, sourceTable).map(sourceDataset => {
        const dataset = datasets.insertOne({
          id: Date.now().toString(),
          data: sourceDataset.data,
          count: sourceDataset.count,
        });
        return tables.insertOne({
          id: Date.now().toString(),
          parentId: Date.now().toString(),
  ...
  },
  {
    ...
    handler: function (params) {
      ...
      return getDataset(this, table).map(dataset => {
        ...
        return Csv.fromArray([table.columns.map(column => column.displayName), ...dataset.data]);
      });
    },
  },
  {
