            ...
            <div class="col-md-12">
              <div class="p0 form-control brand-control">
                <div class="row" *ngVar="customLoaders | apply: _filterLoaders: entity; let loaders = value">
                  ...
                  <div class="col-xs-6" *ngIf="showVersionTable">
                    <a class="link" (click)="toggleVersionTable(null)">
                      <i class="glyphicon glyphicon-arrow-left"
                        tooltip
                        data-toggle="tooltip"
                        tooltipTitle="Back to all tables"></i>
                    </a>
                    <span class="ellipsis text-bold">
                    {{tableName}}
                    </span>
                  </div>
                  <div class="col-xs-6"
                    [ngClass]="{'col-xs-offset-6': !loaders.length && !showVersionTable}"
                  >
          ...
          <div *ngIf="items && !(itemsDataLoading | async)" class="table-scroll" [adaptiveHeight]="{minHeight: 450, pageMargin: 150}">
            <table class="table dataTable table-bordered table-hover">
              ...
              <tr style="white-space: nowrap;">
                <ng-container *ngIf="showVersionTable">
                  <th style="width: 8%">Version</th>
                  <th>Description</th>
                </ng-container>
                <th *ngIf="!showVersionTable"
                ...
                <th *ngIf="this.entity === config.asset.values.TABLE && !showVersionTable"
                style="width: 8%;"
                >Versions</th>
                <th
              ...
              <tr *ngFor="let item of items.data">
                <ng-container *ngIf="showVersionTable">
                  <td style="width: 5%">
                    <a class="link" (click)="setValue(item)">{{item.version}}</a>
                  </td>
                ...
                <td *ngFor="let column of entity | apply: _getColumns">
                  {{item | apply: column.get: asyncData[entity]: additionalItemData}}
                </td>
                <td *ngIf="this.entity === config.asset.values.TABLE && !showVersionTable">
                  <ng-container [ngSwitch]="(item | apply: _checkValidity: entity: itemFilter) && (item | apply: canViewVersions: entity)">
                    <div *ngSwitchCase="true"
                      style="text-align: center;">
                      <a (click)="toggleVersionTable(item)"
                        class="link">
                        {{item.version}}
...
export class LibrarySelectorComponent extends WithProjectSelectionHelpers implements OnChanges, OnInit, OnDestroy {
  ...
  showVersionTable: boolean = false;

  projects: IProject[] = [];
  readonly _projectsLoader: ReactiveLoader<IBackendList<IProject>, any>;
  readonly itemsLoader: ReactiveLoader<[IItemListPartial, any], void>;
  ...
  constructor(
    ...
    this.itemsLoader.subscribe(([items, additionalItemData]) => {
      this.items = items;
  ...
  toggleVersionTable(item: ITable | null): void {
    if (this.showVersionTable) {
      this.showVersionTable = false;
      this.setCustomLoader(null);
    } else {
      ...
      let loader = {
        name: 'Versions of Table',
        entity: this.entity,
        list: (params) => section.service.versions(parentId, params),
      };
      this.setCustomLoader(loader);
    }
  }

  canViewVersions = (item: IAsset, entityType: IAsset.Type): boolean =>
    entityType === IAsset.Type.TABLE
    ? this.acl.canViewVersions(<ITable> item)
    : false;
  ...
  private _loadList(): Observable<[IItemListPartial, any]> {
    ...
    const items$ = this._customLoader && this._customLoader.entity === this.entity
      ? this._customLoader.list(listRequest)
      : section.service.list(listRequest);
    if (section.loadAdditionalItemData) {
      return items$.flatMap(
        _ => section.loadAdditionalItemData(_).map((aid): [IItemListPartial, any] => [_, aid]),
      );
    }
    return items$.map((list): [IItemListPartial, any] => [list, undefined]);
  }

  private _loadSelectedName(): Observable<string> {
    if (this.value && this.value.object) {
      return Observable.of(this.perhapsFormatName(this.value.object));
      ...
      return observable.map(_ => this.perhapsFormatName(_));
    } else {
      return Observable.of('-Select-');
    }
  }

  private perhapsFormatName(item: IAsset): string {
    if (this.entity === IAsset.Type.TABLE) {
      this.tableName = item.name || this.tableName;
      return `${item.name || this.tableName} (version: ${(<ITable> item).version})`;
    } else {
      return item.name;
    }
  }
