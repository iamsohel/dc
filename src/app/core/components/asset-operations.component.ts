      ...
      <div class="col-xs-12 flex-static">
        ...
        <ul class="asset-btn-panel nav nav-pills">
          ...
          <li *ngIf="type === config.asset.values.TABLE
            ...
            || type === config.asset.values.MODEL
          ...
          <li class="nav-item" [ngClass]="{'disabled': disabledEdit}" [routerLinkActive]="['active']">
            <ng-template [ngIf]="!disabledEdit">
              ...
              <a *ngIf="type === config.asset.values.TABLE"
                class="nav-link link-colorless"
                [routerLink]="['/desk', 'library', 'tables', selectedItems[0].id, 'edit']">
                <i class="imgaction glyphicon glyphicon-pencil center-block"></i>
                <div>edit</div>
              </a>
            </ng-template>
          ...
          <ng-template [ngIf]="sections[type]">
            ...
            <ng-template [ngIf]="type === config.asset.values.TABLE && showVersion">
              <li class="nav-item"
                [ngClass]="{'disabled': shouldDisableVersions() }"
                [routerLinkActive]="['active']"
                [ngSwitch]="shouldDisableVersions()">
                <a *ngSwitchCase="false" class="nav-link link-colorless"
                  [routerLink]="versionRoute">
                  <i class="glyphicon fa fa-history imgaction center-block"></i>
                  <div>Versions</div>
                </a>
                <a *ngSwitchDefault class="nav-link link-colorless">
                  <i class="glyphicon fa fa-history imgaction center-block"></i>
            ...
            <li *ngFor="let action of sections[type].actions | keys"
              class="nav-item"
              [ngClass]="{
                'disabled': selectedItems | apply: _isActionDisabled: sections[type].actions[action],
...
export class AssetOperationsComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes['scope'] && changes['scope'].currentValue !== changes['scope'].previousValue) {
      this.generatePreviewLink();
    }
  }
  ...
  updateDisabledFlags() {
    const readOnly = !!this.selectedItems.find((item) => {
      switch (this.type) {
        ...
        case IAsset.Type.MODEL:
          return !this.acl.canUpdateModel(<IModel> item);
        ...
        case IAsset.Type.DC_PROJECT:
          return !this.acl.canUpdateDCProject(<IDCProject> item);
    ...
    this.disabledInfo = this.selectedItems.length !== 1
      ...
      || (
        this.type === IAsset.Type.TABLE && this.isEditPage()
      );
    ...
    this.disabledEdit = this.selectedItems.length !== 1 || readOnly
      ...
      || (this.type === IAsset.Type.TABLE && (!this.acl.canEditTable(<ITable> this.selectedItems[0]) || this.isEditPage()));
  }

  isAssetTypeShareable(assetType: IAsset.Type): boolean {
    const nonShareableAssets = [
      IAsset.Type.ONLINE_JOB,
      IAsset.Type.DC_PROJECT,
    ];
  ...
  generatePreviewLink() {
    ...
    if (this.selectedItems[0]) {
      switch (this.type) {
        case IAsset.Type.TABLE:
          let table = <ITable> this.selectedItems[0];
          this.versionRoute = ['/desk', 'library', 'tables', (this.scope || 'personal'), table.parentId, 'versions'];
          this.previewRoute = ['/desk', 'library', 'tables', (this.scope || 'personal'), table.id];
          break;
        case IAsset.Type.MODEL:
        case IAsset.Type.CV_MODEL:
        case IAsset.Type.DATASET:
      ...
      const observables: Observable<any>[] = (<any[]> items).map((item: any): Observable<any> => {
        ...
        switch (this.type) {
          ...
          case IAsset.Type.FLOW:
            ...
            break;
          case IAsset.Type.MODEL:
            observable = this.models.delete(<IModel> item);
            break;
          case IAsset.Type.CV_MODEL:
          ...
          case IAsset.Type.ONLINE_JOB:
            ...
            break;
          case IAsset.Type.DC_PROJECT:
            observable = this.dcProjects.delete(<IDCProject> item);
            break;
          case IAsset.Type.SCRIPT_DEPLOYMENT:
  ...
  clone() {
    if (this.selectedItems.length === 1) {
      this.cloneModal.open(this.type, this.selectedItems[0]);
    }
  }
    ...
    if (this.selectedItems[0]) {
      switch (this.type) {
        ...
        case IAsset.Type.CV_MODEL:
          ...
          break;
        case IAsset.Type.MODEL:
          this.downloadObserver.observe(this.models.download(this.selectedItems[0].id));
          break;
        case IAsset.Type.DATASET:
  ...
  _isActionDisabled = (selectedItems: IAsset[], action: LibrarySectionDefinition.Action): boolean => {
    return selectedItems.length !== 1 || (action.isAvailable && !action.isAvailable(selectedItems[0]));
  };
  ...
  shouldDisableVersions = (): boolean =>
    this.selectedItems.length === 1 && this.type === IAsset.Type.TABLE
    ? !this.acl.canViewVersions(<ITable> this.selectedItems[0])
    : true;

  isEditPage(): boolean {
    return this.router.url.split('/').pop() === 'edit';
  }
