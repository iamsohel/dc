...
export class NoQueryEncoder extends HttpUrlEncodingCodec implements HttpParameterCodec {
  encodeKey(k: string): string {
    return k;
  }
...
export class AppHttp {
  ...
  getTokenPme() {
    const token = this.storage.get(config.storage.pmeToken);
    return token ? config.api.pmeTokenValue(token.access_token) : null;
  }
  ...
  //PME
  getpme(url: string, params?: any, options?: AppHttpRequestOptions): Observable<any> {
    let res = this._http.request('GET', config.api.pme + url, this._optionsPme(options, params));
    return this._catch(this._deserialize(res, options), options).share(); // share: to make cosecutive observers calls
  }

  postpme(url: string, body?: any, params?: any, options?: AppHttpRequestOptions): Observable<any> {
    let res = this._http.request('POST', config.api.pme + url, this._optionsPme(options, params, body));
    return this._activityObserver.observe(this._catch(this._deserialize(res, options), options).share()); // share: to make cosecutive observers calls
  }

  postpmedownload(url: string, body?: any, params?: any, options?: AppHttpRequestOptions): Observable<any> {
    let res = this._http.request('POST', config.api.pme + url, this._optionsPmeDownload(options, params, body));
    return this._activityObserver.observe(this._catch(this._deserialize(res, options), options).share()); // share: to make cosecutive observers calls
  }

  getpmedownload(url: string, params?: any, options?: AppHttpRequestOptions): Observable<any> {
    let res = this._http.request('GET', config.api.pme + url, this._optionsPmeDownload(options, params));
    return this._catch(this._deserialize(res, options), options).share(); // share: to make cosecutive observers calls
  }

  putpme(url: string, body: any, params?: any, options?: AppHttpRequestOptions): Observable<any> {
    let res = this._http.request('PUT', config.api.pme + url, this._optionsPme(options, params, body));
    return this._activityObserver.observe(this._catch(this._deserialize(res, options), options).share()); // share: to make cosecutive observers calls
  }

  patchpme(url: string, body: any, params?: any, options?: AppHttpRequestOptions): Observable<any> {
    let res = this._http.request('PATCH', config.api.pme + url, this._optionsPme(options, params, body));
    return this._activityObserver.observe(this._catch(this._deserialize(res, options), options).share()); // share: to make cosecutive observers calls
  }
  ...
  uploadPme(url: string, bodyParams: any, options: AppHttpRequestOptions = {} ) {
    let body = new FormData();
    body.append('file', bodyParams.file, bodyParams.file.name);
    body.append('columns_to_update', bodyParams.columns_to_update);

    let res = this._http.request('POST', config.api.pme + url, this._uploadOptionsPme(options, {}, body));
    return this._activityObserver.observe(this._catch(this._deserialize(res, options), options).share());
  }

  webSocket<T>(url: string, onOpen: NextObserver<Event>, onClose: NextObserver<CloseEvent>): WebSocketSubject<T> {
    const token = this.storage.get(config.storage.token).access_token;
    return new WebSocketSubject({
      url: config.api.base.replace(/^http/, 'ws') + url + '?access_token=' + token,
      openObserver: onOpen,
  ...
  private _optionsPme(options: AppHttpRequestOptions = {}, params?: Object, body?: any): AngularHttpClientRequestOptions {
    const token = this.getTokenPme();
    const serviceHeaders = {};
    if (token) {
      serviceHeaders[config.api.tokenLabel] = token;
    }
  ...
  private _optionsPmeDownload(options: AppHttpRequestOptions = {}, params?: Object, body?: any): AngularHttpClientRequestOptions {
    const token = this.getTokenPme();
    const serviceHeaders = {};
    if (token) {
      serviceHeaders[config.api.tokenLabel] = token;
  ...
  private _uploadOptionsPme(options: AppHttpRequestOptions = {}, params?: Object, body?: any): AngularHttpClientRequestOptions {
    const token = this.getTokenPme();
    const serviceHeaders = {};
    if (token) {
      serviceHeaders[config.api.tokenLabel] = token;
    ...
    if (!options || !options.noNotifications) {
      out = out.catch((error: AppHttpError) => {
        if (error.message) {
          ...
          this.notifications.create('Something wrong!', config.notification.level.values.DANGER);
        }
        return Observable.empty();
      });
    }
  ...
  private _encodeHttpParams(params?: Object): HttpParams {
    ...
    return new HttpParams({ encoder: new NoQueryEncoder(), fromObject: paramsMap });
  }
