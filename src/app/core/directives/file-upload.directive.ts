...
export class FileUploadDirective implements OnInit, OnDestroy {
  ...
  @Input() accept: string[] = config.table.import.accept;
  @Input() sizeLimit: number = 4 * 1024 * 1024 * 1024; // 4Gb
    ...
    Object.keys(files).forEach((key: string) => {
      ...
      if (this.sizeLimit && file.size > this.sizeLimit) {
        this.notificationService.create(
          ...
          that exceeds the allowable limit of ${MiscUtils.formatBytes(this.sizeLimit)}`,
